...
export default function Transactions() {
    ...
    const loadAccounts = async () => {
        try {
            ...
            const userAccounts = isAdmin
                ? allAccounts
                : allAccounts.filter(
                      (account) => account.user_id === currentUser.id
                  );
            ...
            if (response.data.data) {
                ...
                setTotalPages(
                    response.data.last_page ||
                        Math.ceil(allTransactions.length / perPage)
                );
                setTotal(
                    isAdmin ? response.data.total : allTransactions.length
                );
            } else {
                setTransactions(allTransactions);
    ...
    const handleAddTransaction = async (e) => {
        ...
        if (!isAdmin) {
            ...
            if (
                !selectedAccount ||
                selectedAccount.user_id !== currentUser.id
            ) {
    ...
    const handleDeleteTransaction = async (id) => {
        ...
        if (!isAdmin) {
            ...
            if (transaction) {
                ...
                if (
                    !transactionAccount ||
                    transactionAccount.user_id !== currentUser.id
                ) {
        ...
        <div style={{ padding: "20px" }}>
            ...
            {!isAdmin && (
                ...
                >
                    ...
                    {accounts.length === 0 ? (
                        <p>
                            You don't have any accounts yet. Create an account
                            first to add transactions.
                        </p>
                    ) : (
                        <div>
                            <p>
                                You have {accounts.length} account
                                {accounts.length !== 1 ? "s" : ""}:
                            </p>
                            <ul>
            ...
            {!loading &&
                ...
                Object.values(filters).every((f) => f === "") && (
                    ...
                    >
                        ...
                        <p>
                            You don't have any transactions yet. Add your first
                            transaction below!
                        </p>
                    </div>
                )}
